...
def encrypt(plain, key):
    ...
    for k in range(len(plain)):
        ...
        if ord(character) <= 90 and ord(character) >= 65: #lowercase
            i = alphabet.index(chr(ord(character)+32)) # index della versione uppercase della lette
            characterEncrypted = chr(ord(key[i])-32) # versione criptata è il char della key alla posizione precedente -32
